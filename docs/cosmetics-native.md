...
- Weapon finish material rebuilds use stable appearance signatures (definition, paint, seed, wear, counter and name), with at most two pending cosmetic changes per normal stage. Identity and lifecycle-flag drift can repair scalar/attribute state without tearing down unchanged materials. No render-frame material construction loop and no per-frame disk work.
...
|---|---:|---|
| Read item paint | `0x110F2D0` | Resolves the literal `set item texture prefab` at `0x1BCA128`, traverses item attributes through virtual `+0xE8`, returns its numeric value. Used only on the verified game thread before rebuilding. |
...
## 3.27.1 finish application correction

The prior weapon path wrote fallback paint/seed/wear fields but did not populate the embedded item-view attributes. The existing glove path already used those attributes. Current offline disassembly confirms that `C_EconItemView`'s paint getter consumes `set item texture prefab`, not the weapon fallback scalar directly. Weapons and knives now commit attributes 6/7/8 through the already pinned engine setter, verify the full attribute readback, and verify the engine paint getter before composite refresh. Engine calls that silently leave a value unchanged no longer count as successful application.

Original attribute presence and values are retained per full weapon handle. Attribute writes roll back partial failures; disable/ownership loss restores only values that still match this adapter's last applied values. A newer server value wins. No stock-value reset, raw vector ownership transfer, or speculative virtual slot was added.

The material request stores the requested appearance separately from mutable engine item state. Stable identity, initialization, SOC and precache-flag drift does not invoke composite teardown. Cheap repairs are limited to one per item every 250 ms. A new selection or new full entity handle schedules a material request. Actual native refresh attempts are bounded; waiting for attributes, the logical paint readback, item construction or a model resource does not consume those attempts. A pending model request is issued once and its resource/name readback is polled instead of repeatedly reloading the same model. `NativeStatus.materialRefreshes` counts actual heavy refresh invocations, including restoration.

The supplied source mesh rules conflict: Anthony's explicit `MeshMask` uses knife legacy=1/modern=2, while Jeremy uses legacy=2/modern=1 for both guns and knives. Vortex retains its existing, working model replacement mapping; this patch does not guess a reversal from those contradictory implementations. Gun legacy=2/modern=1 agrees in both sources.

The focused Release fixture and optional installed-archive run pass **73 checks, zero failures**. New cases cover paint/seed/wear attribute population, no-op writes, absent-attribute restoration, partial native failures, preservation of newer server values, silent setter failures, delayed resources, 1,000 stable simulated frames with identity/flag drift, changed wear/seed, and bounded retry recovery. The installed catalog remains 207 definitions, 1,481 finishes and 2,121 valid pairs. These are offline checks; in-game finish appearance has not been visually validated during this fix.

Pending model transitions keep the issued source/target separate from the last confirmed resident model. Disabling during an outstanding application waits for its readback, then requests the original model once. A slow restoration retains ownership while the previous replacement is still resident; it completes only after the original model is confirmed (or the full entity handle is gone/a distinct server model supersedes ownership). Agent fast paths also acknowledge completed transitions without discarding the original snapshot. Eight additional transition checks cover ten seconds of simulated application/restoration delay, disabling mid-load, successful restoration and external model changes; the five central freshness-grace checks are included in the 73-check total above.
